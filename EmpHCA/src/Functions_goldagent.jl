...
function softmax!(x)
    ...
    if max_val == Inf
        # Limiting case: an unbounded parameter (e.g. βm) can push a logit to
        # +Inf during optimization; Inf - Inf below would otherwise produce
        # NaN and silently corrupt every entry. Softmax's limit as one logit
        # → ∞ is uniform mass over the +Inf entries, zero elsewhere.
        n_inf = 0
        for val in x
            if val == Inf
                n_inf += 1
        ...
        for i in eachindex(x)
            x[i] = x[i] == Inf ? inv_n : zero(eltype(x))
        end
        return x
    end
...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# The order of the room in Figure S1 of the paper is different from the code
...
# rooms used only in experiments 2 and 3
const valid_rooms_E23 = [0,1,2,4,5,6,7,8,9] .+ 1;
export valid_rooms_E23
...
function room_information(; Ymax = 1, Xmax = 1)
    Prooms, ΔState, ΔStateDict = gold_proom_sets();
    N_rooms = length(Prooms); 
    return Prooms, ΔState, ΔStateDict, N_rooms, Ymax, Xmax
end
export room_information
...
function gold_pa1_EmpNa!(X, Na, emp_model, β, βa, agent)
    for i = eachindex(agent.pa)
        agent.pa[i] = 
            β * p2empCat!(X[i].p, X[i].sloc, emp_model, X[i].Q, X[i].V) +
            βa * Na[i]
    end
    softmax!(agent.pa)
end
export gold_pa1_EmpNa!
...
# ------------------------------------------------------------------------------
# Xs[t] = (Xs[t][1] , Xs[t][2])
# Xs[t][i] is a (; p, sloc, Q, V) NamedTuple from gold_Room2X_basic/gold_Room2X_indexbased
